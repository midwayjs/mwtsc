...
const { debug, output, colors } = require('../util');
...
function createCLIMessage(result, callback) {
  ...
  output(colors.yellow('*'.repeat(80)));
  output(getLocalizedString('versionWarn'));
  output('');
  for (const item of result) {
    output(
      `${colors.yellow('➜')}  ${item.name}: ${
        item.current
      } (${getLocalizedString('allow')}: ${item.allow.join(', ')})`
    );
  }
  output('');
