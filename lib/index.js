...
const fs = require('fs');
const { replaceTscAliasPaths, prepareSingleFileReplaceTscAliasPaths } = require('tsc-alias');
...
function run() {
  ...
  if (tsconfig.compilerOptions) {
    ...
    // 判断是否需要启动 tsc-alias
    hasPaths = !!(tsconfig.compilerOptions.paths && Object.keys(tsconfig.compilerOptions.paths).length > 0);
  }
