          ...
          (serverReportOption, isFirstCallback, during) => {
            if (isFirstCallback) {
              ...
              if (serverReportOption && serverReportOption.port) {
                console.log(
                  `\x1B[32m➜\x1B[0m  Local:    \x1B[36mhttp://127.0.0.1:\x1B[1m${serverReportOption.port}/ \x1B[0m`
                );
                const netWorkIp = getIp();
                if (netWorkIp) {
                  console.log(
                    `\x1B[32m➜\x1B[0m  \x1B[2mNetwork:  http://${netWorkIp}:${serverReportOption.port}/ \x1B[0m`
                  );
                }
                console.log('');
              }
